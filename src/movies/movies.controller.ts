...
export class MoviesController {
    ...
    @Put()
     updateMovie(
        ...
        @Body() createMovieDTO: CreateMovieDTO) {
            const updatedMovie = this.movieSvc.updateMovie(movieID, createMovieDTO);
            if (!updatedMovie) {
                throw new NotFoundException('Movie does not exist!');
            }
            return res.status(HttpStatus.OK).json({
    ...
    @Delete()
     deleteMovie(@Res() res, @Query('movieID', new ValidateObjectId()) movieID) {
        const deletedMovie = this.movieSvc.deleteMovie(movieID);
        if (!deletedMovie) {
            throw new NotFoundException('Movie Does not exist!');
        }
        return res.status(HttpStatus.OK).json({
